...
def test_basic_functionality():
    ...
    try:
        ...
        print("[OK] 成功创建MemoryAnalyzerV1_8_Optimized实例")
        
        # 测试关键词库
        keywords = analyzer.keywords
        if keywords and isinstance(keywords, dict):
            print(f"[OK] 关键词库加载成功，包含{len(keywords)}个类别")
        else:
            print("[ERROR] 关键词库加载失败")
            return False
    ...
    try:
        ...
        for i, test in enumerate(test_cases, 1):
            ...
            if predicted_type == test["expected_type"]:
                ...
                print(f"[ERROR] 测试用例{i}: 类型识别错误 (预测: {predicted_type}, 期望: {test['expected_type']})")
        
        accuracy = correct / total
        print(f"[INFO] 准确率: {accuracy:.1%} ({correct}/{total})")
...
def test_performance():
    ...
    try:
        ...
        print(f"[INFO] 分析结果: {json.dumps(result, ensure_ascii=False, indent=2)}")
    ...
    try:
        ...
        if os.path.exists(backup_path):
            ...
            try:
                ...
                print(f"[OK] 备份版本功能正常，分析结果: {test_result['type']}")
                
                return True
                
            except ImportError as e:
                print(f"[ERROR] 备份版本导入失败: {e}")
                return False
        else:
            print(f"[ERROR] 备份版本文件不存在: {backup_path}")
            return False
            
    except Exception as e:
        print(f"[ERROR] 回滚能力测试失败: {e}")
        return False
