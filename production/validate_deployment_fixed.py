...
def check_version_consistency():
    ...
    try:
        ...
        print(f"  [INFO] 配置版本: {config_version}")
        print(f"  [INFO] 备份版本: {backup_version}")
        ...
        print(f"  [INFO] V1.8_optimized文件: {'[OK] 存在' if v18_exists else '[ERROR] 缺失'}")
        print(f"  [INFO] V1.7_final_backup文件: {'[OK] 存在' if v17_exists else '[ERROR] 缺失'}")
        
        return v18_exists and v17_exists
        
    except Exception as e:
        print(f"  [ERROR] 版本一致性检查失败: {e}")
        return False
...
def generate_deployment_report():
    ...
    print(f"  [OK] 部署报告已保存: {report_path}")
...
def main():
    ...
    if passed == total:
        ...
        print(f"\n[警告] 有{total-passed}个验证失败，请修复问题后重新部署")
        return 1
