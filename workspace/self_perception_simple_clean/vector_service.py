...
# 半衰期配置表（天）
KNOWLEDGE_TYPE_HALF_LIFE = {
    "evergreen":       float("inf"),  # 永不过期
    "core":            float("inf"),   # 核心记忆
    "lesson":          365,            # 经验教训
    "user_preference": 180,           # 用户偏好
    "project":         60,            # 项目
    "decision":        90,            # 决策
    "default":         30,            # 默认30天
}
...
def to_decay_lambda(half_life_days: float) -> float:
    """将半衰期转换为指数衰减率 λ = ln(2) / half_life_days"""
    if half_life_days <= 0 or math.isinf(half_life_days):
        return 0.0
    return math.log(2) / half_life_days


def calculate_decay_multiplier(age_in_days: float, half_life_days: float) -> float:
    """计算时间衰减因子: e^(-λ * age)，半衰期后天数越多衰减越大"""
    if half_life_days <= 0 or math.isinf(half_life_days):
        return 1.0
    if age_in_days < 0:
        age_in_days = 0.0
    lambda_h = to_decay_lambda(half_life_days)
    return math.exp(-lambda_h * age_in_days)


def is_evergreen_memory(knowledge_type: str, importance: float, mem_type: str) -> bool:
    """判断是否为常青记忆（不参与时间衰减）"""
    if knowledge_type == "evergreen":
        return True
    if importance >= 0.9:
        return True
    return False
...
def get_half_life_for_memory(knowledge_type: str, mem_type: str) -> float:
    """根据记忆的 knowledge_type 获取对应半衰期"""
    if knowledge_type in KNOWLEDGE_TYPE_HALF_LIFE:
        return KNOWLEDGE_TYPE_HALF_LIFE[knowledge_type]
    if mem_type in KNOWLEDGE_TYPE_HALF_LIFE:
        return KNOWLEDGE_TYPE_HALF_LIFE[mem_type]
    return KNOWLEDGE_TYPE_HALF_LIFE.get("default", 30)


def compute_final_score(cosine_sim: float, importance: float,
                        decay_multiplier: float,
                        sim_weight: float = 0.7,
                        imp_weight: float = 0.3) -> float:
    """综合评分: 0.7 * cosine_sim + 0.3 * importance * decay_multiplier"""
    # importance 原始是 1-5，归一化到 0-1
    norm_importance = importance / 5.0
    return sim_weight * cosine_sim + imp_weight * norm_importance * decay_multiplier


class VectorStorage:
    def __init__(self, db_path: str = r"C:\Users\Administrator\.openclaw\production\data\vector_store\vectors.db"):
    ...
    def search_memories(self, query: str, limit: int = 10, mem_type: str = None,
                        decay_enabled: bool = False) -> List[Dict[str, Any]]:
        """语义搜索，自动过滤已过期记录，支持按 type 筛选，支持时间衰减"""
        self._purge_expired()
        now = datetime.now()
        try:
            conn = sqlite3.connect(self.db_path)
            ...
            if query_vec is not None:
                ...
                for row in rows:
                    ...
                    if vec is not None and len(vec) == len(query_np):
                        cosine_sim = cosine_similarity(query_np, vec)
                        mem["_cosine_sim"] = cosine_sim
                        scored.append((cosine_sim, mem))
                scored.sort(key=lambda x: x[0], reverse=True)

                # 如果开启衰减：对每个结果计算 age_in_days → decay_multiplier → final_score
                if decay_enabled:
                    for _, (cosine_sim, mem) in enumerate(scored):
                        created_str = mem.get("created_at", "")
                        try:
                            created_dt = datetime.fromisoformat(created_str.replace("Z", "+00:00").replace("+00:00", ""))
                            age_in_days = (now - created_dt).total_seconds() / (24 * 3600)
                        except Exception:
                            age_in_days = 0.0

                        importance_val = float(mem.get("importance") or 1)
                        knowledge_type = mem.get("knowledge_type") or ""
                        mem_type_val = mem.get("type") or ""

                        if is_evergreen_memory(knowledge_type, importance_val / 5.0, mem_type_val):
                            decay_mult = 1.0
                        else:
                            half_life = get_half_life_for_memory(knowledge_type, mem_type_val)
                            decay_mult = calculate_decay_multiplier(age_in_days, half_life)

                        final_score = compute_final_score(cosine_sim, importance_val, decay_mult)
                        mem["_age_in_days"] = round(age_in_days, 2)
                        mem["_decay_multiplier"] = round(decay_mult, 4)
                        mem["_half_life"] = half_life if not math.isinf(half_life) else None
                        mem["_final_score"] = round(final_score, 4)

                    # 按 final_score 重新排序
                    scored.sort(key=lambda x: x[1].get("_final_score", 0), reverse=True)

                results = []
                for cosine_sim, mem in scored[:limit]:
                    mem["metadata"] = json.loads(mem["metadata"]) if mem["metadata"] else {}
                    mem["similarity_score"] = round(cosine_sim, 4)
                    if decay_enabled:
                        mem["final_score"] = mem.pop("_final_score", round(cosine_sim, 4))
                        mem["age_in_days"] = mem.pop("_age_in_days", 0)
                        mem["decay_multiplier"] = mem.pop("_decay_multiplier", 1.0)
                        mem["half_life_days"] = mem.pop("_half_life", None)
                        mem["decay_enabled"] = True
                    strip_vector(mem)
                    results.append(mem)
                conn.close()
                return results
            ...
            for row in cur.fetchall():
                ...
                if decay_enabled:
                    ...
                    if is_evergreen_memory(knowledge_type, importance_val / 5.0, mem_type_val):
                        ...
                        half_life = get_half_life_for_memory(knowledge_type, mem_type_val)
                        decay_mult = calculate_decay_multiplier(age_in_days, half_life)
                    final_score = compute_final_score(0.0, importance_val, decay_mult)
                    mem["age_in_days"] = round(age_in_days, 2)
                    mem["decay_multiplier"] = round(decay_mult, 4)
...
async def search_memories(query: str, limit: int = 10, type: str = None, decay_enabled: bool = False):
    memories = await run_in_threadpool(vector_storage.search_memories, query, limit, type, decay_enabled)
    return {"query": query, "count": len(memories), "memories": memories,
            "decay_enabled": decay_enabled, "timestamp": datetime.now().isoformat()}
