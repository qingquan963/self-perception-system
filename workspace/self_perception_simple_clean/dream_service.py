...
DEFAULT_DREAM_CONFIG = {
    ...
    "execution": {"max_duration_minutes": 20, "batch_size": 30, "concurrency": 2, "dry_run": False, "cpu_threshold": 0.3, "memory_threshold_mb": 500},
        ...
        try:
            ...
            for phase_name in ["deduplicator", "denoiser"]:
                ...
                if not ok or cpu > cpu_limit or mem_mb > mem_limit:
                    logger.warning(f"[Dream] 资源不足 CPU={cpu:.1f}% MEM={mem_mb:.0f}MB，跳过 {phase_name}")
                    report["issues"].append(f"资源不足，跳过 Phase {phase_name}")
                    continue
